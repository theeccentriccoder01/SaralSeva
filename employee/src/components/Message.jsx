...
const Message = () => {
  const { uniqueRecipients, id, getUniqueRecipientsWithLatestMessage } = useContext(EmployeeContext);
              ...
              <div key={data?.recipient?._id} className={`flex gap-4 p-4 cursor-pointer border-l-4 ${selectedRecipient?._id === data?.recipient?._id ? "bg-amber-50 border-amber-500" : "border-transparent hover:bg-gray-50"}`} onClick={() => setSelectedRecipient(data?.recipient)}>
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="text-sm text-gray-500 truncate">
                    {data?.latestMessage?.senderType === "employee" ? "You: " : ''}
          ...
          <>
            ...
            <div className="flex-grow p-4 overflow-y-auto bg-orange-50/30">
              {conversation.map((msg, index) => (
                <div key={index} className={`flex my-2 ${msg.senderType === "employee" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl ${msg.senderType === "employee" ? "bg-orange-600 text-white rounded-br-none" : "bg-gray-200 text-black rounded-bl-none"}`}>
                    {msg.message}
                  </div>
                </div>
              ))}
