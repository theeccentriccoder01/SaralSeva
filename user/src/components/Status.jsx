    ...
    <div className="flex flex-col items-center min-h-[80vh] py-12 px-4 bg-orange-50/30 dark:bg-gray-900/30 transition-colors duration-500">
      <div className="w-full max-w-2xl p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl transition-colors duration-500">
        ...
        <div className="flex bg-gray-200 dark:bg-gray-700 rounded-lg p-1 my-8 transition-colors duration-500">
          <button
            onClick={() => { setChecked("scheme"); setResultScheme(null); setError(''); }}
            className={`flex-1 p-2 rounded-md font-semibold transition-colors ${
              checked === 'scheme' ? 'bg-orange-600 text-white shadow' : 'text-gray-600 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600'
            }`}
          >
            Scheme Status
          </button>
          <button
            onClick={() => { setChecked("grievance"); setResultScheme(null); setError(''); }}
            className={`flex-1 p-2 rounded-md font-semibold transition-colors ${
              checked === 'grievance' ? 'bg-orange-600 text-white shadow' : 'text-gray-600 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600'
            }`}
          >
            Grievance Status
          </button>
        ...
        <div className="mt-8 min-h-[100px]">
          {error && (
            <p className="p-4 text-center text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/30 rounded-lg transition-colors duration-500">{error}</p>
          )}
          {resultScheme && (
            <div className="p-6 bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 dark:border-blue-400 rounded-r-lg transition-colors duration-500">
              <h3 className="text-xl font-bold text-blue-900 dark:text-blue-300">Application Status</h3>
